...
const InvoiceItems = require('../models/invoice_items.model')
    ...
    const validationForm = require('../validations/invoice_items.validation')
    
class InvoiceItemsController {
    getAll = () => {
        return (req, res, next) => {
            InvoiceItems.getAll().then(data => {
                res.status(200).json({
                    rows : data
                })
            }).catch(err => {
    ...
    report = () => {
        return (req, res, next) => {
            InvoiceItems.report(req.query).then(data => {
    ...
    getById = () => {
        return (req, res, next) => {
            ...
            InvoiceItems.getById(id).then(data => {
    ...
    getByColumn = () => {
        return (req, res, next) => {
            ...
            InvoiceItems.getByColumn(column, value).then(data => {
    ...
    createOne = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                InvoiceItems.createOne(data).then(data => {
    ...
    updateByID = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                InvoiceItems.updateByID(data, id).then(data => {
                    res.status(200).json({new_data : data})
                }).catch(err => {
                    res.status(400).json(err)
                })
        ...
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                if (req.query.column || req.query.value) {
                    const column = req.query.column
                    const value = req.query.value
                    InvoiceItems.updateByColumn(data, column, value).then(data => {
                        res.status(200).json({new_data : data})
                    }).catch(err => {
                        res.status(400).json(err)
                    })
    ...
    createList = () => {
        return (req, res, next) => {
            ...
            InvoiceItems.createList(list).then(data => {
                res.status(200).json({rows : data})
            }).catch(err => {
                res.status(400).json(err)
            })
    ...
    deleteList = () => {
        return (req, res, next) => {
            ...
            if (body.list && body.list.length > 0) {
                InvoiceItems.deleteList(body.list)
                    .then(data => {
                        if (data == 1) {
                            res.status(200).json({
