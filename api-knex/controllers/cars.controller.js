...
const Cars = require('../models/cars.model')
    ...
    const validationForm = require('../validations/cars.validation')
...
class CarsController {
    ...
    report = () => {
        return (req, res, next) => {
            Cars.report(req.query).then(data => {
    ...
    getByColumn = () => {
        return (req, res, next) => {
            ...
            Cars.getByColumn(column, value).then(data => {
    ...
    updateByID = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                Cars.updateByID(data, id).then(data => {
                    res.status(200).json({new_data : data})
                }).catch(err => {
                    res.status(400).json(err)
                })
        ...
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                if (req.query.column || req.query.value) {
                    const column = req.query.column
                    const value = req.query.value
                    Cars.updateByColumn(data, column, value).then(data => {
                        res.status(200).json({new_data : data})
                    }).catch(err => {
                        res.status(400).json(err)
                    })
    ...
    createList = () => {
        return (req, res, next) => {
            ...
            Cars.createList(list).then(data => {
                res.status(200).json({rows : data})
            }).catch(err => {
                res.status(400).json(err)
            })
