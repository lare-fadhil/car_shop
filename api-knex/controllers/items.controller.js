...
const Items = require('../models/items.model')
    ...
    const validationForm = require('../validations/items.validation')
...
class ItemsController {
    ...
    report = () => {
        return (req, res, next) => {
            Items.report(req.query).then(data => {
    ...
    getByColumn = () => {
        return (req, res, next) => {
            ...
            Items.getByColumn(column, value).then(data => {
    ...
    createOne = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                Items.createOne(data).then(data => {
    ...
    updateByID = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                Items.updateByID(data, id).then(data => {
                    res.status(200).json({new_data : data})
                }).catch(err => {
                    res.status(400).json(err)
                })
        ...
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                if (req.query.column || req.query.value) {
                    const column = req.query.column
                    const value = req.query.value
                    Items.updateByColumn(data, column, value).then(data => {
                        res.status(200).json({new_data : data})
                    }).catch(err => {
                        res.status(400).json(err)
                    })
    ...
    createList = () => {
        return (req, res, next) => {
            ...
            Items.createList(list).then(data => {
                res.status(200).json({rows : data})
            }).catch(err => {
                res.status(400).json(err)
            })
