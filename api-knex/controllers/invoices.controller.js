...
const Invoices = require('../models/invoices.model')
    ...
    const validationForm = require('../validations/invoices.validation')
    
class InvoicesController {
    getAll = () => {
        return (req, res, next) => {
            Invoices.getAll().then(data => {
                res.status(200).json({
                    rows : data
                })
            }).catch(err => {
    ...
    getByColumn = () => {
        return (req, res, next) => {
            ...
            Invoices.getByColumn(column, value).then(data => {
    ...
    createOne = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                Invoices.createOne(data).then(data => {
    ...
    updateByID = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                Invoices.updateByID(data, id).then(data => {
                    res.status(200).json({new_data : data})
                }).catch(err => {
                    res.status(400).json(err)
                })
        ...
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                if (req.query.column || req.query.value) {
                    const column = req.query.column
                    const value = req.query.value
                    Invoices.updateByColumn(data, column, value).then(data => {
                        res.status(200).json({new_data : data})
                    }).catch(err => {
                        res.status(400).json(err)
                    })
    ...
    createList = () => {
        return (req, res, next) => {
            ...
            Invoices.createList(list).then(data => {
                res.status(200).json({rows : data})
            }).catch(err => {
                res.status(400).json(err)
            })
